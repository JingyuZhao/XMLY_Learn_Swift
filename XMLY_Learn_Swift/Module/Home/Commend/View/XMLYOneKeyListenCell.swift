...
class XMLYOneKeyListenCell: UICollectionViewCell {
    private var oneKeyListen:[XMLYOneKeyListenModel]?
    ...
    private lazy var gridView: UICollectionView = {
        ...
        layout.itemSize = CGSize(width: (XMLYScreenWidth - 45) / 3, height:120)
        let collectionView = UICollectionView.init(frame:.zero, collectionViewLayout: layout)
        collectionView.delegate = self
        collectionView.dataSource = self
        collectionView.backgroundColor = UIColor.white
        collectionView.alwaysBounceVertical = true
        collectionView.register(XMLYOneKeyCell.self, forCellWithReuseIdentifier: "LBOneKeyListenCell")
...
extension XMLYOneKeyListenCell : UICollectionViewDelegate,UICollectionViewDataSource{
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
           return self.oneKeyListen?.count ?? 0
       }
       
       func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
           let cell:XMLYOneKeyCell = collectionView.dequeueReusableCell(withReuseIdentifier: "OneKeyListenCell", for: indexPath) as! XMLYOneKeyCell
