...
class XMLYHomeLiveRankCell: UICollectionViewCell {
    private var multidimensionalRankVosList: [XMLYMultidimensionalRankVosModel]?
    
    
    private let XMLYLiveRankCellID = "XMLYLiveRankCell"
    ...
    private lazy var collectionView: UICollectionView = {
        ...
        layout.itemSize = CGSize(width: (XMLYScreenWidth - 30), height:self.frame.size.height)
        //        layout.sectionInset = UIEdgeInsetsMake(0, 0, 0, 0)
        layout.scrollDirection = UICollectionView.ScrollDirection.horizontal
        let collectionView = UICollectionView.init(frame:.zero, collectionViewLayout: layout)
        collectionView.contentSize = CGSize(width: (XMLYScreenWidth - 30), height: self.frame.size.height)
        ...
        collectionView.register(XMLYLiveRankCell.self, forCellWithReuseIdentifier:XMLYLiveRankCellID)
...
extension XMLYHomeLiveRankCell : UICollectionViewDataSource,UICollectionViewDelegate,UIScrollViewDelegate{
    func collectionView(_ collectionView: UICollectionView, numberOfItemsInSection section: Int) -> Int {
        return (self.multidimensionalRankVosList?.count ?? 0)*100
    }
    
    func collectionView(_ collectionView: UICollectionView, cellForItemAt indexPath: IndexPath) -> UICollectionViewCell {
        let cell:XMLYLiveRankCell = collectionView.dequeueReusableCell(withReuseIdentifier: XMLYLiveRankCellID, for: indexPath) as! XMLYLiveRankCell
